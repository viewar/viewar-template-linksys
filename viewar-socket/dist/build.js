...
var _Subject = __webpack_require__(61);
...
function createSocketConnection(_ref) {
  var joinSession = function () {
    var _ref7 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(_ref8) {
      var id = _ref8.id,
          _ref8$prefix = _ref8.prefix,
          prefix = _ref8$prefix === undefined ? '' : _ref8$prefix,
          _ref8$role = _ref8.role,
          role = _ref8$role === undefined ? 'Client' : _ref8$role;
      var room;
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _context.prev = 0;
              room = id || Math.random().toString(36).substring(7);
              _context.next = 4;
              return this.joinRoom(prefix + room, role);
  ...
  function connect(host) {
    ...
    this.clientLeft$ = new _Subject.Subject();
    this.clientJoined$ = new _Subject.Subject();
    this.clientListUpdate$ = new _Subject.Subject();

    return new Promise(function (resolve, reject) {
      _this.socket = (0, _socket2.default)(host);
      _this.socket.on('connect', function () {
        _this.connected = true;
      });
      _this.socket.on('connect_timeout', function () {
  ...
  function joinRoom(room, role) {
    ...
    return new Promise(function (resolve, reject) {
      var callbackId = 'temp_' + Date.now() + '_' + Math.random().toString(36).substring(7);
      var req = _this2.socket.emit('joinRoom', { room: room, role: role }, callbackId);

      _this2.socket.on('clientJoined', function (client) {
        _this2.clients.push(client);
        _this2.clientJoined$.next(client);
        _this2.clientListUpdate$.next();
      });

      _this2.socket.on('clientLeft', function (clientId) {
        _this2.clients = _this2.clients.filter(function (_ref2) {
          var id = _ref2.id;
          return id !== clientId;
        });
        _this2.clientLeft$.next(clientId);
        _this2.clientListUpdate$.next();
      });

      _this2.socket.on('clientChangedData', function (alteredClient) {
        var remainingClients = _this2.clients.filter(function (_ref3) {
          var id = _ref3.id;
          return id !== alteredClient.id;
        });
        _this2.clients = [].concat(_toConsumableArray(remainingClients), [alteredClient]);
        _this2.clientListUpdate$.next(alteredClient);
      });

      req.once(callbackId, function (_ref4) {
        var err = _ref4.err,
            res = _ref4.res;
        ...
        _this2.clients = res.clients;
        _this2.room = res.name;
        resolve();
      });
    });
  }
  ...
  function leaveRoom(room) {
    ...
    this.socket.off('clientJoined', function (client) {
      _this3.clients.push(client);
      _this3.clientJoined$.next(client);
    });

    this.socket.off('clientLeft', function (clientId) {
      _this3.clients = _this3.clients.filter(function (_ref5) {
        var id = _ref5.id;
        return id !== clientId;
      });
      _this3.clientLeft$.next(clientId);
    });
  ...
  function setClientData(data) {
    if (!this.connected) throw new Error('not connected');
    this.socket.emit('setClientData', { room: this.room, data: data });
  ...
  function getRooms() {
    ...
    return new Promise(function (resolve, reject) {
      var callbackId = 'temp_' + Date.now() + '_' + Math.random().toString(36).substring(7);
      var req = _this4.socket.emit('getRooms', callbackId);
      req.once(callbackId, function (rooms) {
        resolve(rooms);
      });
    });
  }
  ...
  function send(_ref6) {
    var _ref6$room = _ref6.room,
        room = _ref6$room === undefined ? this.room : _ref6$room,
        data = _ref6.data,
        _ref6$messageType = _ref6.messageType,
        messageType = _ref6$messageType === undefined ? 'message' : _ref6$messageType;

    if (!this.connected) throw new Error('not connected');
    if (room) {
      this.socket.emit('send', { room: room, data: data, messageType: messageType });
    } else {
  ...
  function getData() {
    ...
    var action = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'message';

    return new _Observable.Observable(function (observer) {
      _this5.socket.on(action, function (data) {
        observer.next(data);
      });
    });
  }
